...
- **Ranking Critic → Claude (cloud):** Scoring and selection require stronger reasoning and coherence judgment. The default model is `claude-haiku-4-5-20251001`, overridable via `ANTHROPIC_MODEL`. Claude is used here precisely because it outperforms smaller local models at nuanced evaluation tasks. Since the Critic only receives sanitized item IDs, sources, and priority scores — not raw personal content — the privacy risk is minimal. Haiku is well-suited for the Critic's scoring task — the prompt is compact (sanitized IDs and priority scores only) and the evaluation is structured, so the lighter model performs as well as Opus at a fraction of the latency and cost.
